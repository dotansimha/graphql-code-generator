...
In ESM the file extension must be appended to named imports. This can be achieved by setting the
codegen config `importExtension` to `'.js'` or `'.ts'` (see `codegen.yml`).
