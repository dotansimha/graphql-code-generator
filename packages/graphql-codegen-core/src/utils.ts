...
export function pickFlag<TConfig, TKey extends keyof TConfig>(
  flag: TKey,
  config: TConfig,
...
export function shouldValidateDuplicateDocuments(
  ...
  if (
    typeof skipDocumentsValidationOption === 'object' &&
...
export function getSkipDocumentsValidationOption(
  options: Types.GenerateOptions,
): Types.SkipDocumentsValidationOptions {
  ...
  const flagFromConfig: Types.SkipDocumentsValidationOptions = pickFlag(
...
export function hasFederationSpec(schemaOrAST: GraphQLSchema | DocumentNode) {
  ...
  if (isDocumentNode(schemaOrAST)) {
    return schemaOrAST.definitions.some(
      def =>
        def.kind === Kind.DIRECTIVE_DEFINITION && federationDirectives.includes(def.name.value),
    );
  }
  return false;
}
