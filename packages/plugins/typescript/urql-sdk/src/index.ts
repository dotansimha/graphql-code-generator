...
import { GraphQLUrqlVisitor } from './visitor';
import { extname } from 'path';
import { RawGraphQLUrqlPluginConfig } from './config';

export const plugin: PluginFunction<RawGraphQLUrqlPluginConfig> = (
  schema: GraphQLSchema,
  documents: Types.DocumentFile[],
  config: RawGraphQLUrqlPluginConfig
) => {
  ...
  const visitor = new GraphQLUrqlVisitor(schema, allFragments, config);
...
export const validate: PluginValidateFn<any> = async (
  ...
  if (extname(outputFile) !== '.ts' && extname(outputFile) !== 'tsx') {
    throw new Error(`Plugin "typescript-urql-sdk" requires extension to be ".ts" or ".tsx"!`);
  }
};

export { GraphQLUrqlVisitor };
