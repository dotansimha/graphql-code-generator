...
describe('React-Query', () => {
  ...
  describe('fetcher: fetch', () => {
    ...
    it('Should generate fetcher field when exposeFetcher is true', async () => {
      ...
      const out = (await plugin(schema, docs, config)) as Types.ComplexPluginOutput;
      expect(out.content).toBeSimilarStringTo(
        `useTestQuery.fetcher = (variables?: TestQueryVariables, dataSource?: { endpoint: string, fetchParams?: RequestInit }) => fetcher<TestQuery, TestQueryVariables>(dataSource.endpoint, dataSource.fetchParams || {}, TestDocument, variables);`
      );
    });
  });
