...
> {
  ...
  public getScalarsImports(): string[] {
    ...
    const imports: {
      [
        source: string
      ]: // `source` is where to import from e.g. './relative-import', 'package-import', '@org/package'
      {
        identifiers: {
          [
            identifier: string
          ]: // `identifier` is the name of import, this could be used for named or default imports
          { asDefault: boolean };
        };
      };
    } = {};
  ...
  private collectInnerTypesRecursively({
    ...
    if (node instanceof GraphQLScalarType) {
      ...
      if (scalarType.type !== 'GraphQLScalarType') {
        throw new Error(
          `${node.name} has been incorrectly parsed as Scalar. This should not happen.`,
        );
      }
        ...
        Field: () => {
          ...
          if (fieldType) {
            ...
            if (namedType instanceof GraphQLScalarType) {
              ...
              if (scalarType.type !== 'GraphQLScalarType') {
                throw new Error(
                  `${namedType.name} has been incorrectly parsed as Scalar. This should not happen.`,
                );
              }
...
function parseOneOfInputValue({
  ...
  | {
      isOneOfInputValue: true;
      realParentDef: TypeDefinitionNode;
...
function collectAndFlattenTypeNodes({
  ...
  typeNodes: Array<
    | { type: 'ListType'; isNonNullable: boolean }
    | { type: 'NamedType'; isNonNullable: boolean; name: string }
  >;
}): void {
