...
import { PythonIntrospectionVisitor } from './introspection-visitor';
...
export const plugin: PluginFunction<PythonPluginConfig> = (
  ...
  const visitor = new PythonVisitor(schema, config);
...
export function includeIntrospectionDefinitions(
  ...
  const visitor = new PythonIntrospectionVisitor(schema, config, typesToInclude);
