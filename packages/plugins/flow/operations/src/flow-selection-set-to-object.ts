...
export class FlowSelectionSetToObject extends SelectionSetToObject<FlowDocumentsParsedConfig> {
  constructor(_scalars: NormalizedScalarsMap, _schema: GraphQLSchema, _convertName: ConvertNameFn, _loadedFragments: LoadedFragment[], _config: FlowDocumentsParsedConfig, _parentSchemaType?: GraphQLNamedType, _selectionSet?: SelectionSetNode) {
  ...
  public createNext(parentSchemaType: GraphQLNamedType, selectionSet: SelectionSetNode): SelectionSetToObject {
    return new FlowSelectionSetToObject(this._scalars, this._schema, this._convertName, this._loadedFragments, this._config, parentSchemaType, selectionSet);
  }
  ...
  protected buildPrimitiveFields(parentName: string, fields: PrimitiveField[]): string | null {
    ...
    const useFlowExactObject = this._config.useFlowExactObjects;
    const useFlowReadOnlyTypes = this._config.useFlowReadOnlyTypes;
