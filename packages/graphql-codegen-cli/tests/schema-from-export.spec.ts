...
describe('Schema From Export', () => {
  ...
  it('should load the schema correctly from module.exports', async () => {
    const result: any = await instance.handle('./tests/test-files/loaders/module-exports.js', null);
    expect(result instanceof GraphQLSchema).toBeTruthy();
  });

  it('should load the schema correctly from variable export', async () => {
    const result: any = await instance.handle('./tests/test-files/loaders/schema-export.js', null);
    expect(result instanceof GraphQLSchema).toBeTruthy();
  });

  it('should load the schema correctly from default export', async () => {
    const result: any = await instance.handle('./tests/test-files/loaders/default-export.js', null);
    expect(result instanceof GraphQLSchema).toBeTruthy();
  });

  it('should load the schema correctly from promise export', async () => {
    const result: any = await instance.handle('./tests/test-files/loaders/promise-export.js', null);
    expect(result instanceof GraphQLSchema).toBeTruthy();
  });
});
