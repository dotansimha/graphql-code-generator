...
let hooks: Types.LifecycleHooksDefinition<string[]> = DEFAULT_HOOKS;
...
async function executeHooks(hookName: string, scripts: string[] = [], args: string[] = []): Promise<void> {
  ...
  for (const script of scripts) {
    debugLog(`Running lifecycle hook "${hookName}" script: ${script} with args: ${args}...`);
    await execShellCommand(`${script} ${args.join(' ')}`);
  }
}

export const lifecycleHooks = {
  registerHooks(_hooks: Partial<Types.LifecycleHooksDefinition<string | string[]>> = {}): void {
