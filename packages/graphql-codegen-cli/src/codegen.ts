...
export async function executeCodegen(
  ...
  async function normalize() {
    ...
    rootDocuments = normalizeInstanceOrArray<Types.OperationDocument>(
      config.documents
    );
    ...
    for (const filename of generateKeys) {
      const output = (generates[filename] = normalizeOutputParam(
        config.generates[filename]
      ));
    ...
    if (
      ...
      Object.keys(generates).some(
        filename =>
          !generates[filename].schema || generates[filename].schema.length === 0
      )
    ) {
      throw new DetailedError(
        "Invalid Codegen Configuration!",
  ...
  const tasks = new Listr<Ctx, "default" | "verbose">(
    [
      ...
      },
      {
        ...
        task: (ctx, task) => {
          const generateTasks: ListrTask<Ctx>[] = Object.keys(generates).map(
            filename => {
              ...
              return {
                ...
                task: async (_, subTask) => {
                  ...
                  const outputSpecificSchemas = normalizeInstanceOrArray<
                    Types.Schema
                  >(outputConfig.schema);
                  let outputSpecificDocuments = normalizeInstanceOrArray<
                        ...
                        task: wrapTask(
                          async () => {
                            ...
                            const result = await cache(
                              ...
                              hash,
                              async () => {
                                const outputSchemaAst = await context.loadSchema(
                                  schemaPointerMap
                                );
                      ...
                      },
                      {
                        ...
                        task: wrapTask(
                          async () => {
                            ...
                            const process = async (
                              outputArgs: Types.GenerateOptions
                            ) => {
  ...
  if (executedContext.errors.length > 0) {
    ...
    const newErr = new AggregateError(
