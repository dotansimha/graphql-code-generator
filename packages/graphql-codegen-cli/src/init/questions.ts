  ...
  return [
    {
      ...
      default: getApplicationTypeChoices(possibleTargets).findIndex(
        c => c.checked
      )
    },
    {
    ...
    },
    {
      ...
      default: answers =>
        answers.targets.includes(Tags.client) ? "codegen.ts" : "codegen.yml",
      validate: (str: string) => {
        ...
        const hasCorrectExtension = [
          ...
          "js",
          "ts"
        ].some(ext => str.toLocaleLowerCase().endsWith(`.${ext}`));
...
export function getApplicationTypeChoices(
  possibleTargets: Record<Tags, boolean>
) {
