...
export async function writeConfig(answers: Answers, config: Types.Config) {
  const YAML = await import("json-to-pretty-yaml").then(m =>
...
export async function writePackage(answers: Answers, configLocation: string) {
  ...
  await Promise.all(
    (answers.plugins || []).map(async plugin => {
      pkg.devDependencies[plugin.package] = await getLatestVersion(
        plugin.package
      );
    })
  );
  ...
  if (answers.introspection) {
    ...
    ] = await getLatestVersion("@graphql-codegen/introspection");
  }

  pkg.devDependencies["@graphql-codegen/cli"] = await getLatestVersion(
    "@graphql-codegen/cli"
  );
