...
const Tree: React.FC<{
  ...
  <div className="mt-6 select-none text-sm text-gray-800 dark:text-gray-300">
    <div className="inline-flex rounded-lg border px-4 py-2 dark:border-neutral-800">{children}</div>
  </div>
);
...
const Folder: React.FC<FolderProps> = React.memo(({ label, name, open, children, defaultOpen, onToggle }) => {
  const indent = useIndent();
...
const File: React.FC<FileProps> = React.memo(({ label, name, active, ...props }) => {
  return (
    <li className={cn('flex list-none', active && 'nx-text-primary-600 contrast-more:nx-underline')}>
