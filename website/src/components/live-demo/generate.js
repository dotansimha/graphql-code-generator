...
export async function generate(config, schema, documents) {
    try {
        const outputs = [];
        const [{load}, {codegen}, {parse}, {pluginLoaderMap}, {presetLoaderMap}] = await Promise.all([
            ...
            dynamic(import('./plugins').then(m => m.pluginLoaderMap), {ssr: false}),
            dynamic(import('./plugins').then(m => m.presetLoaderMap), {ssr: false}),
        ]);
