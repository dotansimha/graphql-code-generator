...
To use the GraphQL Code Generator with Java, start by adding the
[com.moowork.node](https://plugins.gradle.org/plugin/com.moowork.node) Gradle plugin to your
`build.gradle`:
...
<Callout>
  Make sure to use the latest version of codegen and the plugins, and replace `RELEVANT_PLUGIN` with
  your plugin name.
</Callout>

Then, create `codegen.yml` file in your root directory, pointing to your schema, and add the plugins
