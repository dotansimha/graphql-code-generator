...
export const useFetchData = <TData, TVariables>(
  ...
  variables?: TVariables | QueryFunctionContext<QueryKey, any>,
  context?: QueryFunctionContext<QueryKey, any>
 ) => Promise<TData>) => {
  // it is safe to call React Hooks here.
  const { url, headers } = React.useContext(FetchParamsContext)

  return async <TPageParam = any>(
    variables?: TVariables | QueryFunctionContext<QueryKey, TPageParam>,
...
> Important if you use `queryClient.refetchQueries(queryKey)` with no variables it will not add the variables to the query, this means either add `{}` to queries with no variables or add some logic to see if the variable is now the context.
