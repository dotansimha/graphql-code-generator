...
### Fragment Unmasking utility types

`UnmaskResultOf<DocumentType>` and `UnmaskFragmentType<FragmentType>` are two utility types available to unmask fragments.

They recursively flatten masked fragments by resolving ` $fragmentRefs` and merging them to the root fragment/operation, in short, they will turn a masked fragment containing fragment references to a flatted/inlined fragment.
