...
GraphQL Code Generator is built as a monorepo, using [pnpm workspaces](https://pnpm.io/workspaces) it means that all scripts and dependencies are located in the root `package.json` of the project.
...
Now that your plugin is configured, you need to make sure pnpm knows about it and links it to the monorepo, so rerun the following command, in the root directory:
...
To do that, make sure everything is built by using `pnpm build` in the root directory, then you can use it in `./dev-test/codegen.ts`, and run `pnpm generate:examples` in the project root directory to run it.

If you would like to test "watch mode" in the same way, you can run `pnpm watch:examples`.
...
<Callout>
  ...
  auto-complete (try running `pnpm generate-json-config` to see it in action) and will generate markdown documentation
...
2. Add your plugin's package as a dependency of the `website/package.json` and run `pnpm i` at the root of the repository.
...
Go ahead to `website` directory and run the website using `pnpm dev`.
