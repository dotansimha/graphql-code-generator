...
Forr bundlers such as Webpack this is a huge as it allows the complied bundle to only contain code that is required at runtime, which means smaller download size when loading code over the network.
...
If you are impatient, you can find [a full runnable and ready to use example in the GraphQL Code Generator reporsitory](https://github.com/dotansimha/graphql-code-generator/tree/master/examples/typescript-esm).
...
Furthemore, you can use the ESM cli version of GraphQL Code Generator by utilizing the command `graphql-codegen-esm` over `graphql-codegen`.
...
Supporting ESM and configuring GraphQL Code Generator and your project for ESM requires some worl, but isn't that hard.
As mentioned before, you can find [a full runnable and ready to use example in the GraphQL Code Generator reporsitory](https://github.com/dotansimha/graphql-code-generator/tree/master/examples/typescript-esm).
