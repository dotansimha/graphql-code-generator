...
Apollo-tooling generates nested field type names using **only field names**, omitting the GraphQL object type
name:
...
To achieve naming similar to apollo-tooling with GraphQL Codegen use the `extractAllFieldsToTypesCompact: true`
...
The setup above closely mimics apollo-tooling (99.98% similarity in our tests) but isn’t an exact match.
The following items may require manual fixes:
