...
<Callout>
  The setup below is using a new version of `near-operation-file` preset which is currently in development.
  ...
  generates: {
    './src/': {
      ...
      presetConfig: {
        extension: '.ts',
        folder: '__generated__',
        // Disable namespace prefix on types imported from other generated files
        importTypesNamespace: '',
