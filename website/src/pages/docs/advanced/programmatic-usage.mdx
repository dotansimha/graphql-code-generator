...
await fs.writeFile(path.join(__dirname, outputFile), output)
...
This usage will not work in a browser environment because the `cli` package depends on Node.js internals and the file system.
</Callout>
