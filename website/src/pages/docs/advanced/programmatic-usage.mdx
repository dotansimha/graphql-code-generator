---
description:
  GraphQL Code Generator also provides a complete programmatic API. The programmatic API can be used
  if you need to customize the execution flow or write a tool that uses the codegen.
---
...
([complete signature](https://github.com/dotansimha/graphql-code-generator/blob/master/packages/utils/plugins-helpers/src/types.ts#L6-L21)):
...
<Callout>
  The `schema` field be a valid `GraphQLSchema` object. If you need to load your GraphQL schema from
  an external source (file, URL), you can use `loadSchema` from `@graphql-tools/load`.
</Callout>
...
<Callout>
  You need to import the plugin in your favorite way; you can also use `await import` to lazy load
  it.
</Callout>
...
You can use one of the tools from [`@graphql-tools`](https://github.com/ardatan/graphql-tools) for
file loading, schema merging, transformations and more.
...
<Callout type="warning">
  This usage will not work in a browser environment because the `cli` package depends on Node.js
  internals and the file system.
</Callout>
