...
The document transform can also be specified by file name. You can create a custom file and pass it
to `documentTransforms`.
...
In this case, you can retrieve the `queryName` from the `config` argument of the `transform`
function as follows:
