---
description:
  Learn how to use the GraphQL Code Generator CLI to generate code from your GraphQL schema and
  operations.
---
...
Triggered every time a file changes when using watch mode. Triggered with two arguments: the type of
the event (for example, `changed`) and the file's path.
...
<Callout>
  Not all the files will be written to the file system, because this is triggered before checking if
  the file has changed since last execution.
</Callout>
...
Triggered after a file is written to the file system. Executed with the path for the file. If the
content of the file hasn't changed since the last execution - this hook won't be triggered.
...
Executed after writing all the files to the file system. Triggered with multiple arguments - paths
for all files.
...
The following hooks are available for a single output file: `beforeOneFileWrite` and
`afterOneFileWrite`.
