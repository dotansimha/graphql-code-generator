...
- **`allowPartialOutputs`** - Allows partial outputs to be written to files if one or more `generates` blocks have errors. If all `generates` blocks fail, treat as a complete failure and return non-zero to the terminal. Default is `false`.
