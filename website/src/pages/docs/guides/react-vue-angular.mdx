...
GraphQL Code Generator provides an unified way to get TypeScript types from GraphQL operations.
The following guide is compatible with [most modern GraphQL clients and frameworks](#compatibility).
...
If you are willing to provide your own fetcher, you can directly jump to the ["Appendix I"](#appendix-i-react-query-with-a-custom-fetcher-setup) and continue the guide once React Query properly setup.
...
Then, start GraphQL Code Generator in watch mode:
...
<Tabs items={['React (Apollo, React Query, URQL)', 'Vue (Apollo, URQL)', 'Angular']}>
...
Finally, unlike most setup GraphQL Client setup, you don't need to append the Fragment definition document to the related Query, you simply need to reference it in your Query definition, as shown below:
