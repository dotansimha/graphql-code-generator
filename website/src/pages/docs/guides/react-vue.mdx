...
- [`enumsAsConst`](/plugins/typescript/typescript#config-api-reference): Generates enum as TypeScript const assertions instead of enum. This can even be used to enable enum-like patterns in plain JavaScript code if you choose not to use TypeScript’s enum construct.
