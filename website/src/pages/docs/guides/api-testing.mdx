...
This tutorial uses GraphQL Yoga, but the practices are applyable to testing any GraphQL server.

<Callout type="info">

Refer to [GraphQL Codegen - Yoga example](https://github.com/dotansimha/graphql-code-generator/tree/feat-esm-client-preset/examples/yoga-tests) for a fully running example using [GraphQL Yoga](https://the-guild.dev/graphql/yoga-server).
...
It can be used in any context, wether it is tests or server to server communication.
