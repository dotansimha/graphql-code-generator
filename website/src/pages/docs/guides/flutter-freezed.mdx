...
This plugin as well as the `graphql-code-generator` is written in JS/TS, and this plugin would likely be used in a Dart environment, the following options are available:

- 🚧 The flutter-freezed-cli (WIP)
- A NodeJS project

### The flutter-freezed-cli
This is currently in progress. Upon it completion, you can install and run the generator globally.

A PR would also be grately appreciated for using `build-runner` instead of using a JS CLI
