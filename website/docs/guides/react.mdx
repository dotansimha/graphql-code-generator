---
id: react
title: React and GraphQL Code Generator
---


GraphQL Code Generator is providing typed code generation for React Query, URQL React, React Apollo Client and other clients.
...
For this reason, GraphQL Code Generator provides a `@graphql-codegen/typescript-react-query` plugin that generated typed hook for each GraphQL operation.
...
<PackageInstall packages={["@graphql-codegen/typescript-react-query"]} />
...
For more advanced configuration (custom fetcher, authentication), please refer to the [plugin documentation](/plugins/typescript-react-query#using-graphql-request).
...
While Apollo and URQL have their own GraphQL Code Generator plugins that generate fully-typed hooks, they are also both compatible with a plugin named `@graphql-codegen/typed-document-node`.

`@graphql-codegen/typed-document-node` plugin is providing the best Developer Experience for Apollo and URQL:
...
<CodeTabs namespace={`guides-react-codetabs`}>

<CodeTab label="React Apollo">
...
In order to have `@graphql-codegen/typed-document-node` working and to avoid code duplication,
we highly recommend to move all `gql` document declaration outside of `.tsx`/`.ts` files.
...
<PackageInstall packages={["@graphql-codegen/typed-document-node"]} />
...
GraphQL Code Generator proposes also two plugins (one for Apollo, one for URQL) that generate fully-typed hooks.
...
Some my prefer this approach to `@graphql-codegen/typed-document-node` since its results in a full abstraction of the query logic and less imports.

However, keep in mind that hooks generation results in:

- greater bundle size
- increase of the number of specific hooks in the given application
...
<PackageInstall packages={["@graphql-codegen/typescript-react-apollo"]} />

For URQL React:

<PackageInstall packages={["@graphql-codegen/typescript-urql"]} />
...
For more advanced configuration (custom fetcher, authentication), please refer to the plugin documentation:
...
## Other React GraphQL clients

Even while using a another GraphQL Cient (not listed above), GraphQL Code Generator still got you covered!

`@graphql-codegen/typescript-operations` is the perfect plugin is your prefer only manipulatin generated TypeScript types.
...
Using `@graphql-codegen/typescript-operations` would generate the type definitions for only defined Query, Mutation, Subscription and Fragment.

Which will give the flexibility to keep your GraphQL client use it as follow:
...
For more information on the `@graphql-codegen/typescript-operations` plugin configuration , please refer to its [documentation](/plugins/typescript-resolvers).
