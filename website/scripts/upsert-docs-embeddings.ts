/**
 * Script from: https://github.com/supabase-community/nextjs-openai-doc-search/blob/ed70e36b109e325da6638fd5fe584019af4cdf47/lib/generate-embeddings.ts
 */
...
async function generateEmbeddings() {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY || !process.env.OPENAI_KEY) {
    return console.log(
  ...
  const docsSources: EmbeddingSource[] = [
    ...(await walk('./src/pages/docs/'))
      .filter(({ path }) => /\.mdx?$/.test(path))
      .map(entry => new MarkdownEmbeddingSource('guide', entry.path)),
  ];

  const pluginSources: EmbeddingSource[] = [
    ...(await walk('./src/pages/plugins/'))
      .filter(({ path }) => /\.mdx?$/.test(path))
      .map(entry => new MarkdownEmbeddingSource('guide', entry.path)),
  ];

  console.log(`Discovered ${docsSources.length + docsSources.length} pages`);

  console.log('Checking which pages are new or have changed');

  for (const embeddingSource of [...docsSources, ...pluginSources]) {
