---
description:
  Learn how to create your own plugins for GraphQL Code Generator. You can use plugins to generate
  code, documents, or even to modify the schema.
title: Validate Plugin Configuration
sidebarTitle: Validate Configuration
---
...
- test for other plugin's existence (for example, if your plugin requires another plugin to function
  correctly)
...
<Callout>
  `outputFile` is the name of the output file, and you can use it to enforce the specific filename
  of a specific file extension.
</Callout>

<Callout>
  `allPlugins` is a list of all plugins requested in this specific output file - use it to create
