...
title: 'Guide: React and Vue'
sidebarTitle: React / Vue
---
...
```ts title="src/App.tsx" {6,8-18,22}
...
```ts title="src/App.tsx" {7, 8-18, 23-27}
import React from "react";
...
```ts title="src/App.tsx" {6,8-18,22-27}
import React from "react";
...
```vue title="src/App.vue" {6,9-21,24}
...
```ts title="src/Film.tsx" {4-11,15,17}
...
```vue title="src/components/FilmItem.vue" {5-12,14,17}
<script setup lang="ts">
import { graphql } from '.../src/gql'
...
```ts title="useGraphQL helper function"
import request from 'graphql-request'
...
```ts title="Custom fetcher function and useGraphQL hook implementation"
